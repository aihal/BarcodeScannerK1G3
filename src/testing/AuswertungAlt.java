...
//		LCD.drawInt( (int)balkenDauer3, 5, 7);
	...
	public static Ziffer werteEineZifferAus(EV3ColorSensor hs, long balkenDauer) {
		...
		balken[0] = Farben.getFarbe( werte.subList(werte.size()/4,  werte.size() - werte.size()/4) );

		while(System.currentTimeMillis() < balkenEnde2){
			werte.add(Farben.missWert(hs));
			Delay.msDelay(2);
		}
		balken[1] = Farben.getFarbe( werte.subList(werte.size()/4,  werte.size() - werte.size()/4) );

		while(System.currentTimeMillis() < balkenEnde3){
			werte.add(Farben.missWert(hs));
			Delay.msDelay(2);
		}
		balken[2] = Farben.getFarbe( werte.subList(werte.size()/4,  werte.size() - werte.size()/4) );

		while(System.currentTimeMillis() < balkenEnde4){
			werte.add(Farben.missWert(hs));
			Delay.msDelay(2);
		}
		balken[3] = Farben.getFarbe( werte.subList(werte.size()/4,  werte.size() - werte.size()/4) );
